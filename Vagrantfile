...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.define "centos7-k8s-master" do |master|
    master.vm.hostname = "centos7-k8s-master"
  ...
  (1..NodeCnt).each do |i|
    config.vm.define "centos7-k8s-node#{i}" do |node| 
      node.vm.hostname = "centos7-k8s-node#{i}"
...
192.168.50.101 k8s-node1
192.168.50.102 k8s-node2
EOF
...
# 쿠버네티스 초기화 명령 실행
kubeadm init --apiserver-advertise-address 192.168.50.100 --pod-network-cidr=20.96.0.0/12
